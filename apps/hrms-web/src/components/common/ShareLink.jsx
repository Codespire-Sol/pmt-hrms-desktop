...
export default function ShareLink() {
    ...
    <div style={{ width: 320 }}>
      ...
      {isLocalhost && (
        <Alert
          ...
          description="Others can't open localhost. Open this app using this PC's network address (e.g. http://192.168.x.x:3000) and share that instead."
        />
      )}
  ...
  return (
    <Popover content={content} title={null} trigger="click" open={open} onOpenChange={setOpen} placement="bottomRight">
      <Button
        type="text"
        icon={<Share2 size={18} color={themeTokens.colors.textSecondary} />}
        style={{ display: 'flex', alignItems: 'center', gap: 6 }}
      >
        <span style={{ fontSize: 13, color: themeTokens.colors.textSecondary }}>Share</span>
      </Button>
    </Popover>
  );
}
